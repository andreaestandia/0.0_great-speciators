---
title: "0.0_create_dataset_hawaii"
...
```{r}
land <-
  sf::st_read(dsn = file.path(data_path, "shapefiles", "hawaii.shp")) %>%
...
save(resident_npoly, file=file.path(data_path, "npoly","hawaii_npoly_resident.RData"))
...
save(resident_npoly, file=file.path(data_path, "dfs","hawaii_npoly_resident.RData"))
```
...
save(breeding_npoly, file=file.path(data_path, "npoly","hawaii_npoly_breeding.RData"))
...
save(breeding_npoly, file=file.path(data_path, "dfs","hawaii_npoly_breeding.RData"))
...
write.csv(npoly, file=file.path(data_path, "dfs", "npoly_hawaii.csv"))
```

Calculate biovariables
```{r}
#load saved polygons
load(file.path(data_path, "dfs","hawaii_npoly_breeding.RData"))
...
write.csv(total, file.path(data_path,"dfs", "bioclimate_hawaii.csv"), row.names = F)
```

Calculate median and max distance (range size)
```{r}
#load the polygons again. If already loaded, skip this step
load(file.path(data_path, "npoly/hawaii_npoly_resident.RData"))
...
write_csv(median, file.path(data_path,"dfs", "median_dist_hawaii.csv"))
write_csv(maxdist, file.path(data_path,"dfs", "max_dist_hawaii.csv"))
```
...
```{r}
# df_hawaii_old <- read_csv(file.path(data_path,"dfs", "total_hawaii.csv")) %>% dplyr::select(species_name, subspecies,islands) 

hawaii_npoly <- read_csv(file.path(data_path, "npoly", "npoly_hawaii.csv"))
...
islands_hawaii <- read_csv(file.path(data_path, "dfs", "islands_hawaii.csv")) %>% dplyr::select(islands, scientific_name)
...
bioclim_hawaii <- read_csv(file.path(data_path, "dfs", "bioclimate_hawaii.csv")) %>% 
...
max_dist_hawaii <- read_csv(file.path(data_path, "dfs", "max_dist_hawaii.csv"))
median_dist_hawaii <- read_csv(file.path(data_path, "dfs", "median_dist_hawaii.csv"))

hawaii_dataset <- hawaii_npoly %>%  
  ...
  left_join(max_dist_hawaii) %>% 
  left_join(median_dist_hawaii) %>%
  left_join(bioclim_hawaii) %>% 
...
hawaii_dataset <- hawaii_dataset %>% unite(scientific_name, c(species_name, subspecies), sep=" ") %>% left_join(ioc, by ="scientific_name") %>% mutate(range=ifelse(is.na(range.x), range.y, range.x))%>% dplyr::select(-range.x,-range.y)

write.csv(hawaii_dataset,file.path(data_path, "dfs/0.0_hawaii.csv"), row.names = F)
```
Now it's time to go through every subspecies range and in a new column named "islands" add a 0 if the subspecies range does not fall with our dataset, and 1 is it does. The new file would be called 0.1_hawaii.csv 

```{r}
 hawaii_0.0 <- read_csv(file.path(data_path, "dfs", "0.0_hawaii.csv"))#%>% unite(scientific_name, c(species_name, subspecies), sep=" ")
# islands_hawaii <- read_csv(file.path(data_path, "dfs", "islands_hawaii.csv")) %>% 
#   dplyr::select(scientific_name, islands)
# 
# hawaii_0.1 <- left_join(hawaii_0.0, islands_hawaii, by="scientific_name") %>% mutate(islands=coalesce(islands.x, islands.y)) %>% dplyr::select(-islands.x, -islands.y) %>%  separate(scientific_name, c("genus", "species", "subspecies")) %>% unite(species_name, c(genus, species), sep=" ")
# 
# write.csv(hawaii_0.1,file.path(data_path, "dfs/0.1_hawaii.csv"), row.names = F)
```

Calculate number of subspecies per species
```{r}
hawaii_0.1_subsp_1 <- 
  read_csv(file.path(data_path, "dfs", "0.1_hawaii.csv")) %>%
...
hawaii_0.1 <- full_join(hawaii_0.1_subsp_1, hawaii_0.1_subsp_2) %>% full_join(hawaii_0.1_sp) %>% 
  group_by(species_name) %>% mutate(nsubsp=max(nsubsp, na.rm=T))

write.csv(hawaii_0.1,file.path(data_path, "dfs", "0.2_hawaii.csv"), row.names = F)
```
...
```{r}
hawaii_0.2 <- read_csv(file.path(data_path, "dfs", "0.2_hawaii.csv"))
...
not_joined_clements <-   hawaii_0.2 %>% filter(species_name %!in% clements$species_name) %>% dplyr::select(species_name)

write.csv(not_joined_clements, file.path(data_path, "dfs", "not_joined_clements_hawaii.csv"), row.names = F)
```

The new manually edited spreadsheet will be called 0.3_hawaii. This file should be ready for analysis!
