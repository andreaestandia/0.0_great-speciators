---
title: "0.0_create_dataset_melanesia"
...
```{r}
land <-
  sf::st_read(dsn = file.path(data_path, "shapefiles", "northernmelanesia.shp")) %>%
...
save(resident_npoly, file=file.path(data_path, "npoly","melanesia_npoly_resident.RData"))
...
save(resident_npoly, file=file.path(data_path, "dfs","melanesia_npoly_resident.RData"))
```
...
save(breeding_npoly, file=file.path(data_path, "npoly","melanesia_npoly_breeding.RData"))
...
save(breeding_npoly, file=file.path(data_path, "dfs","melanesia_npoly_breeding.RData"))
...
write.csv(npoly, file=file.path(data_path, "dfs", "npoly_melanesia.csv"))
```

Calculate biovariables
```{r}
#load saved polygons
load(file.path(data_path, "dfs","melanesia_npoly_breeding.RData"))
...
write.csv(total, file.path(data_path,"dfs", "bioclimate_melanesia.csv"), row.names = F)
```

Calculate median and max distance (range size)
```{r}
#load the polygons again. If already loaded, skip this step
load(file.path(data_path, "npoly/melanesia_npoly_resident.RData"))
...
write_csv(median, file.path(data_path,"dfs", "median_dist_melanesia.csv"))
write_csv(maxdist, file.path(data_path,"dfs", "max_dist_melanesia.csv"))
```
...
```{r}
df_melanesia_old <- read_csv(file.path(data_path,"dfs", "total_melanesia.csv")) %>% dplyr::select(species_name, subspecies,islands) 

melanesia_npoly <- read_csv(file.path(data_path, "npoly", "npoly_melanesia.csv"))
...
islands_melanesia <- read_csv(file.path(data_path, "dfs", "islands_melanesia.csv")) %>% dplyr::select(islands, scientific_name)
...
niche <-read_csv(file.path(data_path,"dfs", "diet_realm_Pigot2020.csv")) %>% rename(species_name=tree_name) %>% 
  separate(species_name, c("genus", "species")) %>% unite(species_name, c("genus", "species"), sep=" ") %>% dplyr::select(species_name, Realm)
...
bioclim_melanesia <- read_csv(file.path(data_path, "dfs", "bioclimate_melanesia.csv")) %>% 
...
max_dist_melanesia <- read_csv(file.path(data_path, "dfs", "max_dist_melanesia.csv"))
median_dist_melanesia <- read_csv(file.path(data_path, "dfs", "median_dist_melanesia.csv"))

melanesia_dataset <- melanesia_npoly %>%  
  left_join(birdlife_tax, by ="species_name") %>% unite(scientific_name, c(species_name, subspecies), sep=" ") %>% left_join(islands_melanesia, by="scientific_name")  %>% left_join(clements, by ="scientific_name") %>% distinct(scientific_name, .keep_all = T) %>% separate(scientific_name, c("genus", "species", "subspecies")) %>% unite(species_name, c(genus, species), sep=" ") %>% left_join(hwi_dataset, by="species_name") %>% left_join(mig_beh, by="species_name") %>% left_join(tip_rates, by="tree_name") %>% 
  left_join(max_dist_melanesia) %>% 
  left_join(median_dist_melanesia) %>%
  left_join(bioclim_melanesia) %>% 
...
melanesia_dataset <- melanesia_dataset %>% unite(scientific_name, c(species_name, subspecies), sep=" ") %>% left_join(ioc, by ="scientific_name") %>% mutate(range=ifelse(is.na(range.x), range.y, range.x))%>% dplyr::select(-range.x,range.y)

write.csv(melanesia_dataset,file.path(data_path, "dfs/0.0_melanesia.csv"), row.names = F)
```
Now it's time to go through every subspecies range and in a new column named "islands" add a 0 if the subspecies range does not fall with our dataset, and 1 is it does. The new file would be called 0.1_melanesia.csv 

```{r}
 melanesia_0.0 <- read_csv(file.path(data_path, "dfs", "0.0_melanesia.csv"))#%>% unite(scientific_name, c(species_name, subspecies), sep=" ")
# islands_melanesia <- read_csv(file.path(data_path, "dfs", "islands_melanesia.csv")) %>% 
#   dplyr::select(scientific_name, islands)
# 
# melanesia_0.1 <- left_join(melanesia_0.0, islands_melanesia, by="scientific_name") %>% mutate(islands=coalesce(islands.x, islands.y)) %>% dplyr::select(-islands.x, -islands.y) %>%  separate(scientific_name, c("genus", "species", "subspecies")) %>% unite(species_name, c(genus, species), sep=" ")
# 
# write.csv(melanesia_0.1,file.path(data_path, "dfs/0.1_melanesia.csv"), row.names = F)
```

Calculate number of subspecies per species
```{r}
melanesia_0.1_subsp_1 <- 
  read_csv(file.path(data_path, "dfs", "0.0_melanesia.csv")) %>%
...
melanesia_0.1 <- full_join(melanesia_0.1_subsp_1, melanesia_0.1_subsp_2) %>% full_join(melanesia_0.1_sp) %>% 
  group_by(species_name) %>% mutate(nsubsp=max(nsubsp, na.rm=T))

write.csv(melanesia_0.1,file.path(data_path, "dfs", "0.2_melanesia.csv"), row.names = F)
```
...
```{r}
melanesia_0.2 <- read_csv(file.path(data_path, "dfs", "0.2_melanesia.csv"))
...
not_joined_clements <-   melanesia_0.2 %>% filter(species_name %!in% clements$species_name) %>% dplyr::select(species_name)

write.csv(not_joined_clements, file.path(data_path, "dfs", "not_joined_clements_melanesia.csv"), row.names = F)
```

The new manually edited spreadsheet will be called 0.3_melanesia. This file should be ready for analysis!
