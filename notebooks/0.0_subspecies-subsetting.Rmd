...
melanesia <- read_csv(file.path(data_path,"nsubsp_melanesia.csv")) %>% 
  ...
  filter(category == "subspecies") %>% 
  mutate(occurrences = n()) %>% 
  mutate(distr_index_final = distr_index*occurrences) %>% 
  arrange(desc(distr_index_final)) 

write.csv(melanesia, "test.csv")
```
...
dataset <- left_join(gs_candidates, hwi, by = "species_name") %>% 
 ...
 # mutate(distr_index2 = occurrences_y*distr_index)
...
melanesia <- checklist %>% unite("species_name", c(genus,species), sep= " ") %>% 
left_join(melanesia_distributionindex, checklist, by="species_name") %>% drop_na(distr_index)
write.csv(melanesia, "melanesiansubsp.csv")
