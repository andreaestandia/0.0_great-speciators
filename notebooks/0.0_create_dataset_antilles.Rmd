---
title: "0.0_create_dataset_antilles"
...
```{r}
land <-
  sf::st_read(dsn = file.path(data_path, "shapefiles", "antilles.shp")) %>%
...
save(resident_npoly, file=file.path(data_path, "npoly","antilles_npoly_resident.RData"))
...
save(resident_npoly, file=file.path(data_path, "dfs","antilles_npoly_resident.RData"))
```
...
save(breeding_npoly, file=file.path(data_path, "npoly","antilles_npoly_breeding.RData"))
...
save(breeding_npoly, file=file.path(data_path, "dfs","antilles_npoly_breeding.RData"))
...
write.csv(npoly, file=file.path(data_path, "dfs", "npoly_antilles.csv"))
```

Calculate biovariables
```{r}
#load saved polygons
load(file.path(data_path, "dfs","antilles_npoly_breeding.RData"))
...
write.csv(total, file.path(data_path,"dfs", "bioclimate_antilles.csv"), row.names = F)
```

Calculate median and max distance (range size)
```{r}
#load the polygons again. If already loaded, skip this step
load(file.path(data_path, "npoly/antilles_npoly_resident.RData"))
...
write_csv(median, file.path(data_path,"dfs", "median_dist_antilles.csv"))
write_csv(maxdist, file.path(data_path,"dfs", "max_dist_antilles.csv"))
```
...
```{r}
# df_antilles_old <- read_csv(file.path(data_path,"dfs", "total_antilles.csv")) %>% dplyr::select(species_name, subspecies,islands) 

antilles_npoly <- read_csv(file.path(data_path, "npoly", "npoly_antilles.csv"))

clements <- 
  read_csv(file.path(data_path, "dfs", "Clements-Checklist-v2019-August-2019.csv")) %>% 
  clean_names() %>% 
  dplyr::select(scientific_name, category, range, order, family)

birdlife_tax <- 
  read_csv(file.path(data_path, "dfs", "birdlife_taxonomy.csv")) %>% 
  dplyr::select(scientific_name) %>% 
  separate(scientific_name, c("genus", "species", "subspecies")) %>% 
  unite(species_name, c(genus, species), sep=" ")

islands_antilles <- 
  read_csv(file.path(data_path, "dfs", "islands_antilles.csv")) %>% 
  dplyr::select(islands, scientific_name)

hwi_dataset <- 
  read_csv(file.path(data_path, "dfs", "dataset_3.csv")) %>%  
  clean_names() %>% 
  dplyr::select(species_name, tree_name, hwi, body_mass_log, habitat, latitude) %>% 
  mutate(latitude= abs(latitude)) %>% 
  mutate(habitat=ifelse(habitat==1, "dense", habitat)) %>% 
  mutate(habitat=ifelse(habitat==2, "semiopen", habitat)) %>% 
  mutate(habitat=ifelse(habitat==3, "open", habitat))

mig_beh <- 
  read_csv(file.path(data_path, "dfs" , "mig_beh.csv")) %>% 
  dplyr::select(species_name,Migratory_status_3) %>% 
  rename(migration = Migratory_status_3)

niche <-
  read_csv(file.path(data_path,"dfs", "diet_realm_Pigot2020.csv")) %>% 
  rename(species_name=tree_name) %>% 
  separate(species_name, c("genus", "species")) %>% 
  unite(species_name, c("genus", "species"), sep=" ") %>% 
  dplyr::select(species_name, Realm)

tip_rates <- 
  read_csv(file.path(data_path,"dfs", "tiprates.csv")) %>%  
  separate(tree_name, c("genus", "species")) %>% 
  unite(tree_name, c("genus", "species"), sep="_")

bioclim_antilles <- 
  read_csv(file.path(data_path, "dfs", "bioclimate_antilles.csv")) %>% 
  ...
  mutate(precipitation_range=ifelse(altitude_range=="-Inf",0,altitude_range)) %>% 
  mutate(temperature_range=ifelse(altitude_range=="-Inf",0,temperature_range))

max_dist_antilles <- read_csv(file.path(data_path, "dfs", "max_dist_antilles.csv"))

median_dist_antilles <- read_csv(file.path(data_path, "dfs", "median_dist_antilles.csv"))

antilles_dataset <- 
  antilles_npoly %>%  
  left_join(birdlife_tax, by ="species_name") %>% 
  unite(scientific_name, c(species_name, subspecies), sep=" ")%>%
  left_join(clements, by ="scientific_name") %>% 
  distinct(scientific_name, .keep_all = T) %>% 
  separate(scientific_name, c("genus", "species", "subspecies")) %>% 
  unite(species_name, c(genus, species), sep=" ") %>% 
  left_join(hwi_dataset, by="species_name") %>% 
  left_join(mig_beh, by="species_name") %>% 
  left_join(tip_rates, by="tree_name") %>% 
  left_join(max_dist_antilles) %>% 
  left_join(median_dist_antilles) %>%
  left_join(bioclim_antilles) %>% 
...
antilles_dataset <- 
  antilles_dataset %>% 
  unite(scientific_name, c(species_name, subspecies), sep=" ") %>% 
  left_join(ioc, by ="scientific_name") %>% 
  mutate(range=ifelse(is.na(range.x), range.y, range.x)) %>% 
  dplyr::select(-range.x,-range.y)

write.csv(antilles_dataset,file.path(data_path, "dfs/0.0_antilles.csv"), row.names = F)
```
Now it's time to go through every subspecies range and in a new column named "islands" add a 0 if the subspecies range does not fall with our dataset, and 1 is it does. The new file would be called 0.1_antilles.csv 

```{r}
 antilles_0.0 <- read_csv(file.path(data_path, "dfs", "0.1_antilles.csv"))#%>% unite(scientific_name, c(species_name, subspecies), sep=" ")
# islands_antilles <- read_csv(file.path(data_path, "dfs", "islands_antilles.csv")) %>% 
#   dplyr::select(scientific_name, islands)
# 
# antilles_0.1 <- left_join(antilles_0.0, islands_antilles, by="scientific_name") %>% mutate(islands=coalesce(islands.x, islands.y)) %>% dplyr::select(-islands.x, -islands.y) %>%  separate(scientific_name, c("genus", "species", "subspecies")) %>% unite(species_name, c(genus, species), sep=" ")
# 
# write.csv(antilles_0.1,file.path(data_path, "dfs/0.1_antilles.csv"), row.names = F)
```

Calculate number of subspecies per species
```{r}
antilles_0.1_subsp_1 <- 
  read_csv(file.path(data_path, "dfs", "0.1_antilles.csv")) %>%
...
antilles_0.1 <- full_join(antilles_0.1_subsp_1, antilles_0.1_subsp_2) %>% full_join(antilles_0.1_sp) %>% 
  group_by(species_name) %>% mutate(nsubsp=max(nsubsp, na.rm=T))

write.csv(antilles_0.1,file.path(data_path, "dfs", "0.2_antilles.csv"), row.names = F)
```
...
```{r}
antilles_0.2 <- read_csv(file.path(data_path, "dfs", "0.2_antilles.csv"))
...
not_joined_clements <-   antilles_0.2 %>% filter(species_name %!in% clements$species_name) %>% dplyr::select(species_name)

write.csv(not_joined_clements, file.path(data_path, "dfs", "not_joined_clements_antilles.csv"), row.names = F)
```

The new manually edited spreadsheet will be called 0.3_antilles. This file should be ready for analysis!
